...
use super::{
    get_tcp_addrs, CUSTOM_ACCEPT_THROTTLE_TIME, CUSTOM_DEFAULT_MTU, CUSTOM_LINGER_TIMEOUT,
    CUSTOM_LOCATOR_PREFIX,
};

pub struct LinkUnicastCustom {
...
unsafe impl Sync for LinkUnicastCustom {}

impl LinkUnicastCustom {
    fn new(socket: TcpStream, src_addr: SocketAddr, dst_addr: SocketAddr) -> LinkUnicastCustom {
        ...
        if let Err(err) = socket.set_linger(Some(Duration::from_secs(
            (*CUSTOM_LINGER_TIMEOUT).try_into().unwrap(),
        ))) {
            tracing::warn!(
        ...
        LinkUnicastCustom {
            ...
            src_addr,
            src_locator: Locator::new(CUSTOM_LOCATOR_PREFIX, src_addr.to_string(), "").unwrap(),
            dst_addr,
            dst_locator: Locator::new(CUSTOM_LOCATOR_PREFIX, dst_addr.to_string(), "").unwrap(),
        }
    }
...
#[async_trait]
impl LinkUnicastTrait for LinkUnicastCustom {
...
impl fmt::Display for LinkUnicastCustom {
...
impl fmt::Debug for LinkUnicastCustom {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Tcp")
...
pub struct LinkManagerUnicastCustom {
    manager: NewLinkChannelSender,
    listeners: ListenersUnicastIP,
}

impl LinkManagerUnicastCustom {
...
#[async_trait]
impl LinkManagerUnicastTrait for LinkManagerUnicastCustom {
    async fn new_link(&self, endpoint: EndPoint) -> ZResult<LinkUnicast> {
        ...
        for da in dst_addrs {
            match self.new_link_inner(&da, iface).await {
                Ok((stream, src_addr, dst_addr)) => {
                    let link = Arc::new(LinkUnicastCustom::new(stream, src_addr, dst_addr));
            ...
            res = accept(&socket) => {
                match res {
                    ...
                    Err(e) => {
                        ...
                        tokio::time::sleep(Duration::from_micros(*CUSTOM_ACCEPT_THROTTLE_TIME)).await;
                    }
